...
class PerformanceMetrics:
    ...
    @staticmethod
    def stat_dashboard(portfolio_values, returns, num_years, window, risk_free_rate=0):
...
class CryptoSelector:
    ...
    def create_date_widgets(self):
        ...
        self.start_date_entry.set_date(datetime.now() - timedelta(days=2))  # Set default date to D-2
    ...
    def submit_strategy(self):
        ...
        if selected_strategy == "EqualWeightStrategy":
            ...
            strategy = MarketCapStrategy(data=self.data_loader, rebalancing_window=rebalancing_window, initial_capital=initial_capital,
        ...
        returns = PerformanceMetrics.calculate_returns(strategy.portfolio_value)

        PerformanceMetrics.stat_dashboard(strategy.portfolio_value, returns, 1, self.window)
