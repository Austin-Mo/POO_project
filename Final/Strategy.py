...
class MarketCapStrategy(AbstractStrategy):
    ...
    def calculate_weights(self):
        ...
        total_market_cap = current_market_caps[
            [coin for coin in self.available_assets if coin in current_market_caps.index]].sum()
...
class PriceWeightedStrategy(AbstractStrategy):
    ...
    def __init__(self, data, rebalancing_window, initial_capital, start_date, end_date):
        ...
        prices = {coin: float(self.data.dataframes[coin].loc[start_date_datetime]['Close']) for coin in
                  available_assets}
    ...
    def calculate_weights(self):
        ...
        self.available_assets = [coin for coin in self.data.symbols_list if
                                 self.backtest.current_date in self.data.dataframes[coin].index]
        weights = {coin: float(self.data.dataframes[coin].loc[self.backtest.current_date]['Close']) for coin in
                   self.available_assets}
