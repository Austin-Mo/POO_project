...
class MarketCapStrategy(AbstractStrategy):
    ...
    def __init__(self, data, market_caps, rebalancing_window, initial_capital, start_date, end_date, selected_cryptos):
        self.data = data
...
class EqualWeightStrategy(AbstractStrategy):
    ...
    def calculate_weights(self):
        ...
        self.available_assets = [coin for coin in self.selected_cryptos if self.backtest.current_date in self.data.dataframes[coin].index]
...
class CryptoSelector:
    ...
    def create_date_widgets(self):
        ...
        self.submit_button = ttk.Button(self.date_frame, text="Submit", command=self.select_cryptos)
    ...
    def select_cryptos(self):
        ...
        # Ajouter une case à cocher pour sélectionner/désélectionner toutes les cryptomonnaies
        self.select_all_var = tk.BooleanVar()
        self.select_all_checkbutton = tk.Checkbutton(self.crypto_frame, text="Select All", variable=self.select_all_var, command=self.select_all_cryptos)
        self.select_all_checkbutton.pack()

        self.select_button = ttk.Button(self.crypto_frame, text="Select", command=self.select)
        self.select_button.pack()

    def select_all_cryptos(self):
        # Si la case à cocher est sélectionnée, sélectionner toutes les cryptomonnaies
        if self.select_all_var.get():
            self.crypto_listbox.select_set(0, tk.END)
        # Sinon, désélectionner toutes les cryptomonnaies
        else:
            self.crypto_listbox.select_clear(0, tk.END)
    ...
    def submit_strategy(self):
        ...
        if selected_strategy == "EqualWeightStrategy":
            ...
            market_caps_df = pd.read_csv(r'C:\Users\morel\PycharmProjects\POO_project\market_caps_data.csv')
            market_caps_df['date'] = pd.to_datetime(market_caps_df['date'])
            market_caps_df.set_index('date', inplace=True)
            strategy = MarketCapStrategy(data=self.data_loader, market_caps=market_caps_df, rebalancing_window=rebalancing_window, initial_capital=initial_capital,
                                         start_date=self.start_date, end_date=self.end_date, selected_cryptos=self.selected_cryptos)
